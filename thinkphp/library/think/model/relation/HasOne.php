...
class HasOne extends OneToOne {
	...
	public function has() {
		...
		return $this->parent->db ()->alias ( 'a' )->whereExists ( function ($query) use ($table, $localKey, $foreignKey) {
			$query->table ( [ 
					$table => 'b' 
			] )->field ( 'b.' . $foreignKey )->whereExp ( 'a.' . $localKey, '=b.' . $foreignKey );
		} );
	}
	...
	public function hasWhere($where = []) {
		...
		return $this->parent->db ()->alias ( $model )->field ( $model . '.*' )->join ( $table . ' ' . $relation, $model . '.' . $this->localKey . '=' . $relation . '.' . $this->foreignKey, $this->joinType )->where ( $where );
	}
