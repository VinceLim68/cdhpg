...
class HasMany extends Relation {
	...
	public function has($operator = '>=', $count = 1, $id = '*', $joinType = 'INNER') {
		...
		return $this->parent->db ()->alias ( 'a' )->join ( $table . ' b', 'a.' . $this->localKey . '=b.' . $this->foreignKey, $joinType )->group ( 'b.' . $this->foreignKey )->having ( 'count(' . $id . ')' . $operator . $count );
	}
	...
	public function hasWhere($where = []) {
		...
		return $this->parent->db ()->alias ( $model )->field ( $model . '.*' )->join ( $table . ' ' . $relation, $model . '.' . $this->localKey . '=' . $relation . '.' . $this->foreignKey )->where ( $where );
	}
